class NeuralNetwork {
  ...
  static mutate(network, amount) {
    network.levels.forEach((level) => {
      // mutating the biases by percentage amount
      for (let i = 0; i < level.biases.length; i++) {
        level.biases[i] = utils.linearInterp(
          level.biases[i],
          Math.random() * 2 - 1,
          amount
        );
      ...
      // mutating the weights by percentage amount
      for (let i = 0; i < level.weights.length; i++) {
        for (let j = 0; j < level.weights[i].length; j++) {
          level.weights[i][j] = utils.linearInterp(
            level.weights[i][j],
            Math.random() * 2 - 1,
            amount
          );
